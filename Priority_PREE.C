/////Priority_Pree Scheduling Algorithm ...............
   ///By , Subrata Mandal
...
void sorting(int[], int[], int[], int[], int);
void input(int[], int[], int[], int[], int);
void calculationCT(int[], int[], int[], int[], int);
void pree_priority();
...
void pree_priority()
{
	...
	printf("\n\t   ***PRIORITY_PREE SCHEDULING_ALGORITHM***");
...
void calculationCT(int at[], int bt[], int ct[], int pr[], int size)
{
    ...
    while (temp > 0)
     {
	    int index = -1;
	    int smallPriority = 1000;

	    for (int j = 0; j < size; j++)
	        {
	          if (at[j] <= newCT && prr[j] < smallPriority && bbt[j] > 0)
	             {
		          smallPriority = prr[j];
		          index = j;
	            }
	...
	if (index == -1)
	    {
	       newCT++;        //  Ideal Time, increment time
	        ...
	        {
	         ct[index] = newCT + 1;       // Process completes in 1 unit of time (preemption)
             newCT = ct[index];
             bbt[index]--;               // Decrement remaining burst time
                 if (bbt[index] == 0) 
                   {
                    temp--;
                    }
